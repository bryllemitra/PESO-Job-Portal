...
try {
    ...
    if ($role == 'admin') {
        ...
        // Employer view: Notifications for applicants applying to their own jobs
        $query = "SELECT n.id, n.job_id, CONCAT(u.first_name, ' ', u.last_name) AS applicant_name, j.title AS job_name, a.status AS application_status, n.is_read, n.created_at, 
                  CONCAT(u.first_name, ' ', u.last_name, ' has applied for ', j.title) AS message
                  FROM notifications n
                  JOIN users u ON n.sender_id = u.id
                  JOIN jobs j ON n.job_id = j.id
                  LEFT JOIN applications a ON n.job_id = a.job_id AND a.user_id = n.recipient_id
                  WHERE n.recipient_id = ? AND j.user_id = ? ORDER BY n.created_at DESC LIMIT ?, ?";
    } else {
    ...
    if ($role == 'admin') {
        ...
        // Employer should only see notifications for their own jobs
        $stmt->bind_param('iii', $user_id, $user_id, $offset, $limit);
    } else {
        $stmt->bind_param('iii', $user_id, $offset, $limit);
    }
