...
if ($user_role === 'admin') {
    ...
    // Employers can view profiles of applicants who applied to their jobs
    if (isset($_GET['id'])) {
        ...
        if ($check_data['applicant_count'] === 0) {
            ...
            header("Location: index.php"); // Or any error page
            exit();
        } else {
        ...
        // If no 'id' is passed, show the employer's own profile
        $user_id = $_SESSION['user_id'];
    }
} else {
    // Default for other roles (like 'user')
    $user_id = $_SESSION['user_id'];
}
