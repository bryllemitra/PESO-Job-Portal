...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    // Collect form data for both user and employer fields
    ...
    // Optional fields (for employer data)
    ...
    // Validate zip code and phone number as before
    ...
    $update_stmt = $conn->prepare("
        ...
        SET first_name = ?, last_name = ?, phone_number = ?, street_address = ?, 
            ...
            gender = ?, birth_date = ?, caption = ?, linkedin_profile = ?, portfolio_url = ?, age = TIMESTAMPDIFF(YEAR, ?, CURDATE())
        WHERE id = ?
    ");
    ...
    // 18 placeholders for user details (including age update)
    $update_stmt->bind_param(
        "sssssssssssssssssi", 
        $first_name, $last_name, $phone_number, $address,
        $barangay, $city, $zip_code, $education_level, $completion_year,
        $school_name, $inclusive_years, $gender, $birth_date,
        $caption, $linkedin_profile, $portfolio_url, $birth_date, $user_id
    );

    // Execute the user details update
    if (!$update_stmt->execute()) {
        echo "<script>alert('Error updating user profile.');</script>";
        exit;
    }
...
<section class="mb-5">
    ...
    <div class="row g-3">
        ...
        <div class="col-md-6">
            ...
            <input type="text" name="company_name" id="company_name" class="form-control" value="<?php echo htmlspecialchars($employer['company_name'] ?? ''); ?>">
        </div>

        <!-- Company Description -->
        <div class="col-md-6">
            <label for="company_description" class="form-label fw-semibold">Company Description</label>
            <textarea name="company_description" id="company_description" class="form-control auto-expand" rows="1"><?php echo htmlspecialchars($employer['company_description'] ?? ''); ?></textarea>
        </div>

        <!-- Company Website -->
        <div class="col-md-6">
            <label for="company_website" class="form-label fw-semibold">Company Website/Social Media Page (URL)</label>
            <input type="url" name="company_website" id="company_website" class="form-control" value="<?php echo htmlspecialchars($employer['company_website'] ?? ''); ?>">
        </div>

        <!-- Location -->
        <div class="col-md-6">
            <label for="location" class="form-label fw-semibold">Location</label>
            <input type="text" name="location" id="location" class="form-control" value="<?php echo htmlspecialchars($employer['location'] ?? ''); ?>">
        </div>
