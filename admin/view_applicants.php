...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action && $user_id && $job_id) {
        ...
        // Update the application status and remark in the database
        $updateQuery = "UPDATE applications SET status = ?, remark = ? WHERE user_id = ? AND job_id = ?";
        $stmt = $conn->prepare($updateQuery);
        $stmt->bind_param("ssii", $status, $remark, $user_id, $job_id);
        $stmt->execute();
        $stmt->close();
